...
public class ComboBoxProduto {
    ...
    private int key;      // Código da String (Chave primaria da tabela Produto)
    
    
    // Método construtor
    public ComboBoxProduto(int key, String value) {
        this.key = key;
        this.value = value;
    }
    ...
    public static int getSelectedIndex(JComboBox combo){
        ...
        if (obj == null)
            ...
            return ((ComboBoxProduto) obj).getKey();// devolve o código da chave selecionada
    }
    ...
    public static void preencheComboBoxProduto(int SelCodigo, JComboBox combo, ArrayList<Produto> lista, Boolean addTodos){
        ...
        // cadastrar sem Marca.
        //if (addTodos)
            //vlista.add(new ComboBoxProduto(0,"Todas"));
            
        // percorre toda a lista de Raças e armazena as informações no Vector vlista;
        for (Produto c: lista){ 
            // criando um ComboBoxMarca
            ComboBoxProduto cmb = new ComboBoxProduto(c.getCodProduto(),c.getNomeProduto());
